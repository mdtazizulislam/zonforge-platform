...
const outputDir = path.resolve('proof/runs/serial-ui-cust-01d')
...
async function capture(routePath, selector, screenshotName) {
  ...
  try {
    ...
    const debugPayload = {
      ...
      bodyHtml: await page.locator('body').innerHTML().catch(() => null),
      consoleErrors,
      pageErrors,
      error: error instanceof Error ? error.message : String(error),
    }
    await fs.writeFile(path.join(outputDir, 'debug-state.json'), `${JSON.stringify(debugPayload, null, 2)}\n`, 'utf8')
    throw error
  }
